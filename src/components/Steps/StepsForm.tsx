...
const SERVICE_OPTIONS: ServiceOption[] = [
  ...
  {
    value: "i129f",
    label: "I-129F · Petition for Fiancé(e)",
    category: "family",
  },
  {
    value: "i485_package",
    label: "I-485 · Adjustment of Status Package",
    category: "adjustment",
  },
  {
    value: "i360_i485",
    label: "I-360 / I-485 · VAWA Adjustment Package",
    category: "adjustment",
  },
  {
    value: "i751",
    label: "I-751 · Removal of Conditions on Residence",
    category: "other",
  },
  {
    value: "i539",
  ...
  {
    value: "i589_i765",
    label: "I-589 / I-765 · Asylum Package",
    category: "other",
  },
  {
    value: "i765_renewal",
    label: "I-765 · Renewal of Work Authorization",
    category: "other",
  },
  {
    value: "i918",
    label: "I-918 · U Visa Nonimmigrant Status",
    category: "other",
  },
  {
    value: "n400_n600",
    label: "N-400 · Naturalization / N-600 · Citizenship",
    category: "other",
  },
];
...
const getInitialFormData = (
  ...
  switch (formId) {
    ...
    case "i485-approved-i130":
      return {
        service: "i485_package",
        adjustment_type: "i130_approved",
        party_count: 1,
      };
    case "i485-approved-i140":
      return {
        service: "i485_package",
        adjustment_type: "i140_approved",
        party_count: 1,
      };
    case "i485-approved-asylum":
      return {
        service: "i485_package",
        adjustment_type: "asylum_approved",
        party_count: 1,
      };
    case "i360-i485-vawa":
      return {
        service: "i360_i485",
        vawa_party: "principal_only",
        party_count: 1,
      };
    case "i360-i485-vawa-child":
      return {
        service: "i360_i485",
        vawa_party: "principal_child",
        party_count: 2,
      };
    case "i539":
...
export function StepsForm() {
  ...
  const validateStep = (stepIndex: number): boolean => {
    ...
    switch (stepData) {
      ...
      case 2:
        ...
        if (!formData.consent)
          newErrors.consent = "You must agree to terms and conditions";
        break;
    }
    setErrors(newErrors);
    ...
    switch (formData.service) {
      case "i130":
        return {
          min: 1,
          max: 3,
          label: "How many beneficiaries are included?",
          hint: "Usually 1. If filing for multiple beneficiaries, set the count (max 3).",
        };
      case "i485_package":
        return {
          min: 1,
          max: 2,
          label: "How many applicants are in this Adjustment package?",
          hint: "Commonly 1 or 2 (e.g., principal + spouse).",
        };
      default:
        return {
          min: 1,
          max: 1,
          label: "How many applicants for this service?",
          hint: "Most services are single-applicant. Set to 1.",
        };
    }
  };
  ...
  const renderServiceStep = () => (
    <StepContainer>
      ...
      >
        ...
        <OptionsContainer>
          {SERVICE_OPTIONS.filter((opt) => opt.category === "family").map(
            (o) => (
              <ChipOption
                key={o.value}
                value={o.value}
        ...
        <OptionsContainer>
          {SERVICE_OPTIONS.filter((opt) => opt.category === "adjustment").map(
            (o) => (
              <ChipOption
                key={o.value}
                value={o.value}
        ...
        <OptionsContainer>
          {SERVICE_OPTIONS.filter((opt) => opt.category === "other").map(
            (o) => (
              <ChipOption
                key={o.value}
                value={o.value}
  ...
  const renderDetailsStep = () => {
    ...
    return (
      <StepContainer>
        ...
        {formData.service === "i485_package" && (
          <Box mb={3}>
            ...
            <RadioGroup
              ...
              onChange={(e) =>
                updateFormData({ adjustment_type: e.target.value })
              }
            >
              <OptionsContainer>
        ...
        {formData.service === "i360_i485" && (
          <Box mb={3}>
            ...
            <RadioGroup
              ...
              onChange={(e) =>
                updateFormData({
                  vawa_party: e.target.value,
                  party_count: e.target.value === "principal_child" ? 2 : 1,
                })
              }
            >
              <OptionsContainer>
        ...
        {formData.service !== "i360_i485" && (
          <Box>
            <Label>{partyConstraints.label}</Label>
            <StyledTextField
              type="number"
              value={formData.party_count}
              onChange={(e) =>
                updateFormData({ party_count: parseInt(e.target.value) || 1 })
              }
              inputProps={{
                min: partyConstraints.min,
                max: partyConstraints.max,
              }}
  ...
  const renderContactStep = () => (
    <StepContainer>
      <StepTitle>
        {isOtherService ? "Step 2" : "Step 3"} · Your contact details
      </StepTitle>
      {errorBanner && <ErrorBanner>{errorBanner}</ErrorBanner>}
      <GridContainer>
        <Box>
        ...
        <Box>
          ...
          <StyledSelect
            ...
            onChange={(e) =>
              updateFormData({ contact_method: e.target.value as string })
            }
  ...
  const renderSummary = () => {
    const serviceLabel =
      SERVICE_OPTIONS.find((o) => o.value === formData.service)?.label || "-";
    const adjustmentLabel = ADJUSTMENT_OPTIONS.find(
      (o) => o.value === formData.adjustment_type
    )?.label;
    const vawaLabel = VAWA_OPTIONS.find(
      (o) => o.value === formData.vawa_party
    )?.label;
    return (
      <StepContainer>
        <StepTitle>
          {isOtherService ? "Step 3" : "Step 4"} · Review Your Information
        </StepTitle>
        <SummaryContainer>
          <SummaryItem>
          ...
          <SummaryItem>
            <strong>People Included:</strong> {formData.party_count}
          </SummaryItem>
          <SummaryItem>
            <strong>Contact:</strong> {formData.full_name} ({formData.email})
          </SummaryItem>
        </SummaryContainer>
        <ActionsContainer>
          <SecondaryButton onClick={handleBack}>Go Back</SecondaryButton>
  ...
  return (
    <FormRoot>
      ...
      <Subtitle>
        Answer a few questions so we can set up the right immigration support
        for you.
      </Subtitle>
